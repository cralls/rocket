...
class PageCache
{
	...
	public static function doYourThing()
	{
        ...
        // This will set compilation to work with Warp
	...
	public static function redirectURI()
	{
        ...
        {
            //$browser = Mage::getModel('warp/mobile')->isMobile();
            if($browser){
                return true;
            }else{
                return false;
        ...
        /**
         * removed since now using mobile caching 
         * with above code
         * @throws Exception
         */
	...
	public static function getCachedPage()
	{
		...
		if(self::$multiStoreView){
			...
			$key .= '_' . self::getStoreCode();
                ...
                if(self::isMobile()){
			self::report("This is a mobile page");
			$key .= '_mobile_';
		}
            ...
            $cacheType = array(Mage::getStoreConfig('dfv/cache/type'));
                ...
                if($cacheType[0] == 'redis'){
                    try {
                        ...
                        die('Redis can not be used as the backend for Warp. Check admin for proper configuration.');
                    }
